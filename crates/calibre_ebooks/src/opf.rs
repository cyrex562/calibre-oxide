...
pub fn parse_opf(xml_content: &str) -> Result<OpfMetadata, OpfError> {
    ...
    let metadata_node = root.children()
    ...
    for node in metadata_node.children() {
        ...
        match tag.as_str() {
            ...
            "creator" => {
                ...
                let is_aut = node.attribute("role")
            ...
            "identifier" => {
                ...
                let scheme = node.attribute("scheme")
...
fn ns_opf<'a, 'input>(node: Node<'a, 'input>) -> &'a str {
    node.lookup_prefix("http://www.idpf.org/2007/opf").unwrap_or("opf")
}
