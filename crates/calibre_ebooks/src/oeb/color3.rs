//! Port of `tinycss.color3.parse_color_string` (issue #583): CSS Color
//! Level 3 value parsing (148 named keywords plus `transparent`, `#rgb`/
//! `#rrggbb`, `rgb()`/`rgba()`/`hsl()`/`hsla()`, and `currentColor`).
//!
//! Like [`super::fonts3`]/[`super::media3`], this is plain string/
//! token parsing, not routed through [`crate::css`]'s object model.
//! Originally implemented privately inside the DOCX writer
//! (`docx::writer::utils`, the only consumer at the time); promoted
//! here so `mobi::utils::convert_color_for_font_tag` (a second real
//! upstream consumer of the exact same `tinycss.color3.parse_color_string`
//! call) can share it instead of needing its own copy.
//!
//! Not routed through a real CSS tokenizer -- `rgb()`/`hsl()` arguments
//! are hand-parsed via `str::split`/`str::strip_prefix` rather than
//! validated token types. This is a disclosed, pre-existing narrowing
//! (not introduced by this move) worth reconsidering if a future caller
//! needs stricter grammar conformance (e.g. rejecting whitespace-only
//! variations `rgb()` itself tolerates).
...
/// `(name, r, g, b, a)` with channels in `0..=255`.
#[rustfmt::skip]
...
mod tests {
    ...
    #[test]
    fn parses_named_keywords_case_insensitively() {
        assert_eq!(
            parse_color_string("AliceBlue"),
            Some(CssColor::Rgba(Rgba {
                red: 240.0 / 255.0,
                green: 248.0 / 255.0,
                blue: 1.0,
                alpha: 1.0
    ...
    fn parses_current_color() {
        assert_eq!(parse_color_string("currentColor"), Some(CssColor::Current));
    }

    #[test]
    fn parses_short_and_long_hex() {
        let short = parse_color_string("#f00").unwrap();
        let long = parse_color_string("#ff0000").unwrap();
    ...
    #[test]
    fn parses_rgb_rgba_hsl_hsla_functions() {
        assert_eq!(
        ...
        let hsl = parse_color_string("hsl(0, 100%, 50%)").unwrap();
        match hsl {
            CssColor::Rgba(c) => {
                assert!((c.red - 1.0).abs() < 1e-9);
                assert!(c.green.abs() < 1e-9);
                assert!(c.blue.abs() < 1e-9);
            }
    ...
    fn rejects_malformed_input() {
        ...
        assert_eq!(parse_color_string("#12j456"), None);
