...
- [ ] books.py (partial -- book-get/set-last-read-position, book-get/update-annotations (issue #485, real annotation storage + merge algorithm via `calibre_db::annotations`), the disk-cache layer (issue #482, `calibre_srv::books_cache::BookCache` -- content-hash keying, staging/final directory lifecycle, reaping sweep). The render pipeline itself is now real (`calibre_ebooks::render_book`, issue #481) but not yet wired to `books_cache`/HTTP -- no `book-manifest`/`book-file` endpoints yet (issue #483), no mathjax asset serving; see `calibre_srv::books`/`books_cache`'s module docs; remainder tracked by issue #427)
