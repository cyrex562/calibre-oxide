...
- [ ] cdb.py (partial -- delete-books, set-cover, set-fields, add-book (issue #424 closed, real metadata sniffing + duplicate detection), copy-to-library (issue #425 closed, real two-library copy/move + duplicate detection, no automerge); no generic cmd dispatcher; see `calibre_srv::cdb`'s module doc; remainder tracked by issue #426)
