...
- [ ] to_html.py (`docx/to_html.rs`'s `DOCXToHTML` is still the provisional sketch, wired into `input/docx_input.rs`; `convert_run`/`convert_p`/`read_page_properties`/`convert_body`/`read_block_anchors` are ported, not yet wired in — `mark_block_runs`, links, frames, tables/numbering markup, TOC, and OPF writing remain — see #130)
