...
- [ ] fields.py (pure field-instruction parsing half ported to `docx/fields.rs` — `Field`/scanner/`parse_hyperlink`/`parse_xe`/`parse_index`/`parse_ref`/`parse_noteref`, cross-validated against Python's own `test_parse_fields` cases — plus `FieldsCollector`, the source-tree-only half of the `Fields` orchestrator (the stack-based `w:fldChar`/`w:fldSimple` walk collecting fields, dispatching each by name to the parsers above), producing `hyperlink_fields`/`xe_fields`/`index_fields`. The open architectural question this was blocked on (real `crate::xmltree` migration vs. a side-table for `parse_xe`'s synthetic bookmark) is resolved — same reframing `index.py`'s `process_index` (#293) already demonstrated: no real tree mutation needed, just a later pass (not yet written) that looks up `field.start`'s eventual HTML position via `object_map`'s reverse lookup and stamps an `id` there. Still needed: that anchor-assignment pass, calling `docx/index.rs`'s `process_index` once every `XE` field's anchor is assigned (a disclosed divergence from Python's single-pass timing — see `fields.rs`'s module docs), and resolving `hyperlink_fields` into real `<a>` elements by extending `to_html.rs`'s `resolve_links` — see #130/#290, the only piece left in the entire #22/#130 effort)
