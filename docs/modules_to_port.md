...
- [ ] books.py (partial -- book-get/set-last-read-position, book-get/update-annotations (issue #485, real annotation storage + merge algorithm via `calibre_db::annotations`), the disk-cache layer (issue #482, `calibre_srv::books_cache::BookCache` -- content-hash keying, staging/final directory lifecycle, reaping sweep, real and tested ahead of the actual renderer); no render_book.py in-browser reader pipeline itself, no book-manifest/book-file HTTP endpoints yet, no mathjax asset serving; see `calibre_srv::books`/`books_cache`'s module docs; remainder tracked by issue #427)
