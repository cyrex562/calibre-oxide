...
- [ ] tables.py (#222, phase 1 done: real `read()` bulk-loading for every standard field -- `OneToOneTable`/`SizeTable`/`UuidTable`/`ManyToOneTable`/`RatingTable`(incl. its real rating=0 cleanup)/`ManyToManyTable`/`AuthorsTable`/`FormatsTable`/`IdentifiersTable`, matching `Cache::field_for`'s exact field set. Deliberately NOT wired into `Cache`/`search.rs`/`view.rs` yet (still on the real, tested, per-call-SQL strategy) -- and the mutation side (`remove_books`/`remove_items`/`rename_item`/`set_links`/`fix_link_table`/`fix_case_duplicates`) isn't ported, since nothing consumes/keeps this in-memory model in sync yet. See `tables.rs`'s module docs for the full phased plan and every disclosed simplification. `fields.py`'s typed wrapper layer -- the next phase -- still isn't started.)
- [x] utils.py
