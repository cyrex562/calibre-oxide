...
- [ ] images.py (see #132; also needs `pt_to_emu` from the reader's `images.py`, already ported — #130. Partial: the self-contained utility layer — `get_image_margins`, `create_filename`, `create_docx_image_markup` — is ported to `docx/writer/images.rs`, plus `Style.img_size`/`img_dimension` (needed by the still-unported `create_image_markup`) ported to `oeb/polish/style.rs`. `ImagesManager` itself (`read_image`/`read_svg`/`add_image`/`create_image_markup`/`create_cover_markup`/`write_cover_block`/`serialize`) is not ported — needs real image byte content from the OEB manifest (`ManifestItem` has no `data` field, and `docx::writer` has no wired content-loading path yet — a real design question), `SvgRasterizer`'s `svg_originals` tracking (not part of the existing `oeb::transforms::rasterize::SvgRasterizer` port), and an `Element::insert`-at-front capability (same gap already flagged for `links.py`'s TOC half))
- [ ] links.py (see #132 — `start_text`, `sanitize_bookmark_name`, `TOCItem`/its `serialize`, and `LinksManager`'s bookmark/hyperlink half (`__init__`, `bookmark_for_anchor`, `bookmark_id`, `serialize_hyperlink`) all ported to `docx/writer/links.rs`. Not yet ported: `LinksManager.process_toc_node`/`.process_toc_links`/`.serialize_toc` — need `oeb::toc::TOC` wiring and an `Element::insert`-at-front capability `docx/writer/xml.rs` doesn't have yet, and their only caller, `Convert.write` (`from_html.py`), isn't ported either)
...
- [ ] styles.py (see #132 — `TextStyle`, `BlockStyle`, `FloatSpec` (CSS -> `w:rPr`/`w:pPr`/`w:framePr`, incl. their own `serialize`/`serialize_properties`), and `DescendantTextStyle` (a run's `w:rPr` overrides relative to its containing block) all ported to `docx/writer/styles.rs`, against the `oeb/polish/style.rs` seam, plus `read_css_block_borders`/`parse_css_length`/`css_font_family_to_docx`/`convert_underline`/`LINE_STYLES`/`bmap`/`is_dropcaps`. `StylesManager`'s `create_text_style`/`create_block_style` dedup cache also ported, as an arena (`TextStyleId`/`BlockStyleId` handles into `Vec<TextStyle>`/`Vec<BlockStyle>`) standing in for Python's shared-object intern cache. Not yet ported: `DOCXStyle`'s hash/dedup base class (moot — `TextStyle`/`BlockStyle`/`DescendantTextStyle` derive `Eq`/`Hash` directly), `CombinedStyle`, and `StylesManager.finalize`/`.serialize` (both need real `Block`/`Run` objects from `from_html.py`, not ported yet)
- [ ] tables.py (see #132; partial: `Border`, `border_style_weight`, `as_percent`, `convert_width`, and a `Border`-repackaging `read_css_block_borders` (thin wrapper over `styles.rs`'s already-ported one) all ported to `docx/writer/tables.rs`. `SpannedCell`/`Cell`/`Row`/`Table` themselves are not ported — a real design question (how the mutually-referencing `Cell`/`Row`/`Table` graph, including nested tables, maps onto an arena-of-ids the way `Blocks`/`StylesManager` already do), not a missing-subsystem block)
