...
- [ ] kepubify.py (issue #543 (epic) -- real finding correcting its own earlier framing: NOT blocked on the full tts.py epic (#167/#645-649), only imports one trivial `lang_for_elem` helper. Split into #651 (Kobo sentence/paragraph span wrapping, `add_kobo_spans` -- the intricate, highest-risk core, same character as tts.py's own `wrap_sentence`) / #652 (CSS processing, `process_stylesheet`, uses already-real `crate::css`) / #653 (HTML markup + title-page/cover handling, uses already-real `oeb::polish::cover::*`, also covers two small missing helpers `insert_self_closing`/`merge_multiple_html_heads_and_bodies`) / #654 (`kepubify_container`/`unkepubify_container` orchestrator, wires into `container.py`'s documented `KEPUBContainer` `todo!()` from issue #161; CLI-only entry points scoped out, no real consumer). None started yet.)
