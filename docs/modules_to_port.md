...
- [ ] books.py (partial -- book-get/set-last-read-position, book-get/update-annotations (issue #485, real annotation storage + merge algorithm via `calibre_db::annotations`); no render_book.py in-browser reader pipeline, no mathjax asset serving; see `calibre_srv::books`'s module doc; remainder tracked by issue #427)
