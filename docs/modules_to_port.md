...
- [ ] images.py (geometry/CSS half + the `Images` struct (real embedded-image extraction from the DOCX zip/linked files, deduplicated naming, resizing via `oeb::transforms::rescale::fit_image`) both ported to `docx/images.rs` — two upstream bugs reproduced faithfully, an EMF-to-raster conversion gap disclosed (no Rust EMF parser) — only `pic_to_img`/`drawing_to_html`/`pict_to_html`/`to_html` (the `w:drawing`/`w:pict` → `<img>` markup generators, need the mutable HTML tree) remain — see #130/#289)
