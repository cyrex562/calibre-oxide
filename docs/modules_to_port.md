...
- [ ] writer.py (issue #565, depends on #564 and the already-closed #553 -- PARTIAL: `calibre_utils::fonts::sfnt::cff::writer::{IndexBuilder,StringsWriter,CharsetsWriter}` are real and tested (the write-side mirrors of #564's `Index`/`Strings`/format-0 `Charset` readers, each round-trip-tested directly against #564's own reader). Still missing: the `Subset` orchestrator (the two-pass Top-DICT offset-patching dance + the interior-mutability wiring `StringsWriter::intern`'s `&mut self` needs to satisfy `dict_data::Dict::compile`'s `&dyn Fn` resolver parameter -- a real design decision documented in the module's own doc comment, not yet made) and `CFFTable`'s container/`sfnt::subset` CFF-branch wiring. Issue stays open.)
