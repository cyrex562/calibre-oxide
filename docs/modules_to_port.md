...
- [x] sqlite_extension.cpp (issue #566 CLOSED -- full semantic port incl. real FTS5 registration: `calibre_db::sqlite_extension::register_fts5_tokenizers` registers real `calibre`/`porter` custom tokenizers via raw `libsqlite3-sys` FFI against `fts5_api`/`fts5_tokenizer` (byte-for-byte struct layouts verified against the actual vendored SQLite amalgamation this crate compiles against, not guessed), including real upstream's own override of the built-in `unicode61` tokenizer name. Wired into `Backend::new` via `register_functions`. Also fixed a real, disclosed gap found while researching this: `fts/connection.rs`/`notes/connection.rs`'s `books_fts`/`books_fts_stemmed`/`notes_fts`/`notes_fts_stemmed` DDL had NO `tokenize=` clause at all (silently using FTS5's default, so the "_stemmed" tables were byte-for-byte identical to their non-stemmed siblings) -- added the real `tokenize = 'calibre remove_diacritics 2'`/`'porter calibre remove_diacritics 2'` clauses matching upstream's bundled `fts_sqlite.sql`/`notes_sqlite.sql` exactly. Real end-to-end tests confirm diacritic-folding and English stemming now genuinely work through real FTS5 MATCH queries, and that stemmed/non-stemmed search now actually differ)
...
- [x] covers.py (issue #116 CLOSED -- split into #595-601, all CLOSED, after
      ...
      plumbing, and this crate already has a real, working headless
      2D-rendering pipeline (`tiny-skia`/`resvg`/`usvg`, see
      `oeb::transforms::rasterize`) that's a genuine `QPainter`
...
- [x] render_html.py (issue #119 — achievable slice CLOSED: `calibre_ebooks::html_cover_fallback` real-ports `render_html_svg_workaround`'s two pure-parsing pre-fallbacks (`extract_cover_from_embedded_svg`, `extract_calibre_cover`), wired into `metadata::epub::get_metadata`, fixing a real pre-existing bug there (an HTML cover manifest item's raw source text was silently mislabeled as image bytes). The final Qt-WebEngine rendering fallback (`render_html_data`/`render_html.py`'s own `main()`) remains genuinely deferred — no browser-engine-grade CSS layout exists anywhere in this crate to build it on (the wry/tao scraper worker has no screenshot/print-to-PDF capability, `pdf::render::Graphics` is blocked on a live Qt paint engine, `covers`/`covers_text` is narrowly generative not general-HTML). `epub_input.rs`'s EPUB-conversion cover-guide generation and `oeb/reader.rs`'s HTML-cover-to-manifest-item resolution are separate, still-unported surrounding features this fallback would eventually plug into — not part of this issue's own scope, tracked as pre-existing gaps, not newly discovered by this port.)
- [x] tweak.py (issue #120 CLOSED -- `crates/calibre_ebooks/src/tweak.rs`: real `explode`/`implode` dispatch, `get_tools`, `zip_exploder`/`zip_rebuilder` (EPUB/HTMLZ, built on already-real `ArchiveHandler::extract` + `zipfile_safe_replace::build_zip_atomic` -- that function's own doc already named `zip_rebuilder` as its intended caller), `docx_exploder` (reuses already-real `docx::dump::pretty_all_xml_in_dir`), and the hidden marker-file round-trip-format-check mechanism. MOBI/AZW/AZW3 delegates to `mobi::tweak` (below). Disclosed, not faked: the interactive terminal confirmation is an injected `question: impl Fn(&str) -> bool` closure rather than real Python's raw-mode single-keypress `termios`/`msvcrt` TTY reading -- a real CLI binary wires up its own terminal loop, tests pass a canned answer. Verified with a real explode-edit-rebuild-reopen round trip on a real EPUB, not just per-function unit tests.)
...
- [x] __init__.py
- [x] input.py (archive extraction + page enumeration (issue #19); the Qt-based `PageProcessor`/`render_pages`/`process_pages` rendering pipeline (issue #124) now CLOSED too -- `crates/calibre_ebooks/src/comic/page_processor.rs`, a real rewrite off Qt onto the `image` crate plus the already-real `calibre_utils::imageops`/`quantize` (issues #569-#571): crop/rotate (landscape page splitting, right-to-left aware), border removal, normalize, aspect-preserving fit-and-pad (`keep_aspect_ratio`/`wide`), sharpen, despeckle, grayscale, and PNG quantization, following the real `PageProcessor.render`/`process_pages` step order exactly. Parallel-job orchestration deliberately simplified per this issue's own scope note: a bounded `std::thread::scope` + shared-queue worker pool (matching this crate's own established `web::feeds::download::build_index` pattern) replaces the real `Server`/`ParallelJob` subprocess orchestration -- no subprocess isolation or progress callbacks, just the real per-page work run concurrently. Disclosed, not reproduced: the final `Format_Indexed8`/`Format_Grayscale16` conversion only changes Qt's own PNG *encoder* bit-depth/palette choice, not any pixel value -- this port always writes standard 8-bit RGB(A) PNG/JPEG, matching `covers.rs`'s own established encoding convention (real output files may be modestly larger, pixel values are unaffected). Disclosed, narrower mechanism, same outcome: "was the source already grayscale" is detected from real pixel content (`r==g==b` everywhere) rather than Qt's own image-format metadata, since `image::load_from_memory` always decodes to a uniform RGBA8 buffer with no analogous format tag to read)
...
- [x] __init__.py
- [x] archives.py
- [x] cli.py (issue #126 CLOSED — `calibre_conversion::cli_helpers` (input/output validation, USAGE banner, HEURISTIC_OPTIONS, issue #20/#476) plus the real option-assembly half, `calibre_conversion::cli_options::ConvertArgs`: a hand-written `clap::Parser` struct populating `calibre_ebooks::conversion::options::ConversionOptions`/`StructureOptions` — a real, disclosed architectural adaptation of upstream's dynamic per-plugin `OptionRecommendation` registry (`option_recommendation_to_cli_option`) to a statically-typed struct, appropriate since every input/output plugin in this port is a fixed, compile-time-known set, not a dynamically-loaded one. Preserves real upstream quirks: `remove_fake_margins`'s real recommended value is `True`, so its real CLI switch is the negating `--disable-remove-fake-margins`, not a plain `--remove-fake-margins`; `verbose` is a repeatable count flag (`-vv`). Wired into `bin/ebook_convert.rs`, verified with a real HTML→EPUB conversion using live CLI flags (`--base-font-size`, `--embed-all-fonts`, `--use-auto-toc`, `-vv`). Real, disclosed narrowing: `HEURISTIC_OPTIONS` (`markup_chapter_headings` etc.) have no CLI flags yet since `HTMLPreProcessor` itself isn't wired into `Plumber::run()` — exposing flags for options with no real destination would be pure theater.)
...
- [x] plumber.py (issue #686 CLOSED — `Plumber::run()`'s transform stage was a no-op placeholder despite `oeb::transforms::*` already having 20 real, tested modules from issue #41; now the real, complete stage sequence runs end to end: `DataURL` → `Clean`(guide) → `RemoveFirstImage`/`MergeMetadata` → `DetectStructure` → `Jacket` → `AddAltText`/`LinearizeTables`/`UnsmartenPunctuation` (option-gated) → `CSSFlattener` → `RemoveFakeMargins`/`RemoveAdobeMargins` → `EmbedFonts`/`SubsetFonts` (option-gated) → `ManifestTrimmer` → `toc.rationalize_play_orders()` (new). New `calibre_ebooks::conversion::options::ConversionOptions` carries the real upstream `OptionRecommendation` default values (not the individual transform modules' own unrelated test-convenience `Default` impls, which several of them diverge from — e.g. real `insert_metadata` defaults to `false`, `JacketOptions::default()`'s own test convenience default is `true`). Verified end to end: a real HTML→EPUB conversion via `Plumber::run()` now produces a real flattened-CSS output (`stylesheet.css` + `calibre*` classes), not the unmodified input markup. Real, disclosed narrowings: no named output/input device-profile catalog (default profile's own constants only), `user_metadata` is always blank (matching real upstream's own default absent CLI overrides — confirmed this really does reset the title to "Unknown", tracing back to `Metadata.__init__`'s own `NULL_VALUES` fallback, a faithfully-replicated real upstream quirk, not a bug), no XPath-expression CLI options beyond `StructureOptions`'s own already-real subset. Output plugin `convert()` signatures not yet taking `opts` was tracked separately and is now closed — see the #690 entry below.)
- [x] conversion/plugins/*_output.py — `convert()` signatures now take `opts: &ConversionOptions` (issue #690 CLOSED). Every dispatched output plugin (`EPUBOutput`, `DOCXOutput`, `MOBIOutput`, `RBOutput`, `LitOutput`, `TXTOutput`, `SnbOutput`, `RTFOutput`, `FB2Output`, `PDFOutput`, `LRFOutput`, `OEBOutput`, `PDBOutput`, `ODTOutput`, `TCROutput`) gained the parameter, and `Plumber::write_output` threads `&self.opts` through at every call site. `MOBIOutput::convert` is the real, demonstrated consumer: it now builds its `MobiWriterOpts` from `opts.mobi` instead of always `MobiWriterOpts::default()` — previously `dont_compress`/`prefer_author_sort`/`share_not_sync`/`mobi_keep_original_images` were silently discarded no matter what a caller set. Also fixed a real, related bug this same option surfaced: `ConversionOptions::flattener_options`'s `needs_old_markup` (`file_type == 'lit' or (file_type == 'mobi' and mobi_file_type == 'old')`) could only ever honor the `lit` half, since there was nowhere to carry `mobi_file_type` — a default `ebook-convert --output mobi` was silently using KF8-style markup instead of the old-format markup real upstream's own default (`mobi_file_type='old'`) implies. New `ConversionOptions::mobi_file_type` (`MobiFileType::{Old,Both,New}`, default `Old`) fixes this for the `Old` case; `Both`/`New` still don't select a joint MOBI6+KF8 writer (a real, separate, already-tracked gap — #157/#161, "No AZW3Output plugin wires this path yet"). `log`/`input_plugin` pull-through was decided against for this pass: nothing in this port's own logging story needs a plugin-visible `log` object yet, and no output plugin's own logic depends on which input plugin produced the `OEBBook` -- both real, disclosed narrowings, not oversights.)
- [x] preprocess.py
- [x] search_replace.py
- [x] utils.py
...
- [ ] tbs_periodicals.rst
- [x] tweak.py (issue #120's own dependency, shipped alongside it -- `crates/calibre_ebooks/src/mobi/tweak.rs`. `explode` is a real, full port: reuses the exact `MobiReader`/`Mobi8Reader` pipeline already established as the MOBI *input* conversion path (`input::mobi_input::MOBIInput::convert`) with `for_tweak: true`, plus real Python's own pre-checks (Topaz/KFX rejection already `MobiReader::new`'s own error branches, DRM rejection via `MobiReader::check_for_drm`, KF8-required rejection for MOBI6-only files, the "joint" MOBI6+KF8 confirmation question as an injected closure). `rebuild` has no real target and says so rather than faking one: no AZW3 output plugin exists anywhere in this crate yet (a pre-existing, already-tracked gap -- issues #157/#161, confirmed via a matching `todo!`-style bail already present in `oeb/polish/create.rs` for the exact same gap -- not something newly discovered or silently worked around here). Building a real "OPF directory -> AZW3 file" entry point is a separate, substantial undertaking out of scope for a `tweak.py` port.)
...
- [x] kepubify.py (issue #543 (epic) CLOSED -- real finding correcting its own earlier framing: NOT blocked on the full tts.py epic (#167/#645-649), only imports one trivial `lang_for_elem` helper. Split into #651 (Kobo sentence/paragraph span wrapping -- CLOSED, `add_kobo_spans`/`unwrap`/`remove_kobo_spans`/`lang_for_elem` in `oeb::polish::kepubify`. The intricate, highest-risk core of the whole epic; cross-validated against upstream's own verbatim Python running on a minimal lxml-shaped tree, 40 fixtures byte-identical for both `add_kobo_spans` and the `remove_kobo_spans` round trip. Real representation win: because a text run is an ordinary node in `crate::dom::Dom` rather than lxml's out-of-band `.text`/`.tail`, upstream's three-way `parent.text` / `parent[at-1].tail` / `insert(at, ...)` split collapses to one in-place splice, and its `try/except ValueError` "wrapped child" fallback disappears entirely. Two unreachable-in-practice upstream quirks in `unwrap` disclosed rather than silently repaired: `del p[idx]` drops the span's tail and any second element child, and an empty span raises `TypeError`.) / #652 (CSS processing -- CLOSED, `oeb::polish::kepubify`: `process_stylesheet`/`nest_css_comments`/`check_if_css_needs_modification`/`make_options`, real `Options` NamedTuple shared with #653/#654. Disclosed encoding narrowing: `crate::css::model::Rule` has no comment variant (`cssparser` discards raw comments during tokenization), so a hidden `@page` rule is round-tripped through a marker unknown-at-rule's quoted-string prelude instead of a real CSS comment -- same observable inertness to any real CSS engine, different on-disk bytes than real calibre's kepub output. Also disclosed: `check_if_css_needs_modification`'s own `-> tuple[bool, bool]` type hint is wrong, it actually returns a 3-tuple; ported the 2-value behavior its only real caller uses.) / #653 (HTML markup + title-page/cover handling -- CLOSED: `add_style_and_script`/`remove_kobo_styles_and_scripts`/`wrap_body_contents`/`unwrap_body_contents`/`is_probably_a_title_page`/`add_dummy_title_page`/`remove_dummy_title_page`/`remove_dummy_cover_image`/`first_spine_item_is_probably_title_page` all real in `oeb::polish::kepubify`, plus the two small missing helpers `insert_self_closing` (`oeb::polish::utils`, a no-op wrapper here since this crate's `Dom` has no lxml-style `.text`/`.tail` bookkeeping to preserve) and `merge_multiple_html_heads_and_bodies` (`oeb::parse_utils`). Two real upstream bugs disclosed and replicated bug-for-bug rather than fixed: `wrap_body_contents`'s own pre-cleanup XPath has a missing-quotes typo that silently no-ops half of it (`@id={INNER_DIV_ID}` lexes as a node-set test, not a string comparison), and `add_dummy_title_page`'s template has a mismatched `<h3>...</h1>` closing tag. `add_kobo_markup_to_html`/`remove_kobo_markup_from_html` landed with #651, which supplied the span core they dispatch into -- #653 is now complete.) / #654 (CLOSED -- `kepubify_container`/`unkepubify_container`/`do_work_in_parallel`/`process_path`/`process_stylesheet_path`/`kepubify_path`/`unkepubify_path`/`uniqify_name`/`check_for_kobo_drm`/`remove_kobo_files` all real in `oeb::polish::kepubify`; wires into `container.py`'s documented `KEPUBContainer` `todo!()` from issue #161 -- `KepubContainer::open_zip` now really unkepubifies on load and `commit_epub` really re-kepubifies a hardlink clone via the same `Container::clone_to` pairing issue #649 established, leaving `self` the plain editable EPUB. Real parallel dispatch uses this crate's own `thread::scope`+shared-queue pattern instead of porting `calculate_number_of_workers`'s exact heuristic (disclosed narrowing of the sizing formula only, not of the parallelism). A real fallback-cover path (`covers::generate_cover`, issue #116) is wired in for books with no cover image at all. Real upstream bug replicated bug-for-bug: `uniqify_name`'s collision-retry branch is `f'{uuid4()}/fname'`, a literal un-interpolated string, not `f'{uuid4()}/{fname}'`. CLI-only entry points (`kepubify_main`/`unkepubify_main`/`develop`/`profile`) scoped out, no real consumer. Tested end to end: a real directory-backed EPUB fixture through `kepubify_container`/`unkepubify_container` (markup add/remove, dummy cover generation, CSS @page/widows hiding verified structurally not by substring), a real zip fixture through the whole-file `kepubify_path`/`unkepubify_path` wrappers, and `get_container`'s `.kepub` dispatch through `KepubContainer::open_zip`+`commit_epub` end to end.)
...
- [x] subset.py
- [x] toc.py
- [x] tts.py (issue #167 (epic) CLOSED -- every sub-issue #645-649 shipped. Split after a full read of the real 576-line file found a genuine multi-part epic: #645 (sentence marking + HTML span wrapping -- CLOSED, `oeb::polish::tts`: `mark_sentences_in_html`/`unmark_sentences_in_html`/`wrap_sentence` and its ten branches of exact tree surgery, the real intricate core of the epic. Unlike `kepubify.py`'s span wrapping (#651), this one genuinely indexes by element position, so rather than restructure it the module carries a small faithful lxml view over `crate::dom::Dom` (`el_children`/`el_index`/`el_insert`/`get_text`/`set_text`/`get_tail`/`set_tail`) and transcribes upstream statement for statement. Cross-validated against upstream's programmatically-extracted verbatim source on a minimal lxml-shaped tree: 54 fixtures, byte-identical trees, sentence lists and unmark round-trips, with all ten `wrap_sentence` branches confirmed exercised by instrumented coverage. Two things that would have been silently wrong and were caught by reading rather than assuming: offsets must be in **characters**, since the normalization `split_into_sentences_for_tts_embed` applies is character-length-preserving but not byte-length-preserving (`PARAGRAPH_SEPARATOR` is U+2029, 1 char / 3 bytes), and a moved element must carry its tail run explicitly, since lxml's tail belongs to its element while this DOM's is a separate sibling. One real upstream bug reproduced and disclosed: when a sentence starts and ends inside one element's text, upstream clears the tail on both that element and its clone, dropping that text from the document (the recorded sentence text is unaffected) -- confirmed by running upstream's own code, not inferred.) / #646 (SMIL media-overlay generation -- CLOSED, `oeb::polish::tts::make_par`, on `crate::xmltree::Xml` since SMIL is strict namespaced XML like OPF/NCX, not tag-soup HTML. Real disclosed narrowing: the unused `container` parameter was dropped. `par`/`text`/`audio` are created with no namespace, matching real upstream -- `seq.makeelement('par')` with a bare tag doesn't inherit an ancestor's default xmlns in lxml, corroborated by upstream's own `remove_embedded_tts` reading them back via `local-name()` rather than a namespace-qualified test. The one attribute needing its prefix preserved for EPUB3 conformance, `epub:textref`, is stored under that literal qualified string -- a narrow, disclosed exception to `Xml`'s documented unprefixed-attribute convention, justified because every other document that convention covers (OPF/NCX/OCF/DOCX) genuinely has no prefixed attributes, unlike SMIL's one.) / #647 (batch TTS synthesis -- CLOSED, `crate::tts::batch::text_to_raw_audio_data`, on the already-real streaming `crate::tts::stream::Piper` from #77. Two real, disclosed narrowings: voice *resolution* (`resolve_voice`/`download_voice`/`load_voice_metadata`, GUI-adjacent config+network plumbing) isn't ported -- this takes an already-resolved `config_path`/`model_path` directly, matching `Piper::set_voice`'s own real parameters; and resampling to a caller-requested sample rate isn't implemented (no resampling infrastructure exists anywhere in this workspace, the same real gap #648 is blocked on) -- audio is always returned at the voice's own native rate, reported back rather than silently mismatched. One disclosed improvement: upstream's own `timeout` parameter is declared but never actually applied to its blocking queue read (confirmed by reading the body); this port's is real and enforced. Tested end-to-end against a real downloaded Piper voice model, not just the empty-text fast path.) / #648 (WAV->M4A/AAC transcoding -- CLOSED, `crate::tts::transcode::wav_to_m4a`. User chose a pure-Rust encoder over shelling out to system ffmpeg or narrowing the output format. A real feasibility spike (own scratch project, ran the candidates' OWN test suites rather than trusting crates.io descriptions) found `oxideav-aac`'s lib.rs doc comment claiming "encode/decode not wired up yet" is stale -- its real `encoder_roundtrip` test suite (24 signal-based tests) passes on this box. Paired with `oxideav-mp4` for the actual M4A container (upstream's own output is AAC muxed into MP4, not bare ADTS). Both genuinely pure Rust, confirmed via an empty `links =` in Cargo.lock. Disclosed narrowings: only 1-6 channel layouts (Table 1.19's PCE-free layouts; irrelevant since every real caller in this crate produces mono TTS audio) and no MP4 edit-list gapless trimming (real SMIL sync timing comes from the raw PCM duration computed before transcoding, in #647, so this doesn't affect sync accuracy). Tested end-to-end against real synthesized Piper speech, not just synthetic sine waves.) / #649 (`embed_tts`/`remove_embedded_tts` orchestrator -- CLOSED, `oeb::polish::tts::embed_tts`/`remove_embedded_tts`, closing #167 entirely. Real per-file pipeline: mark sentences, batch-synthesize per (lang,voice) group, concatenate into one WAV per file, transcode to M4A, build a real SMIL doc via `make_par`, wire `media-overlay` onto the HTML manifest item, accumulate `media:duration` metadata. `resolve_voice`/`report_progress` are real dependency-injected closures replacing GUI-adjacent voice-download plumbing this crate never ported (same line #647 already drew); an unresolvable `(lang, voice)` silently skips just those sentences, matching upstream's own `if duration > 0` skip for empty text. Disclosed narrowing: one sample rate per file, since #647/#648's own no-resampling narrowing means mixing two voices' raw PCM at different native rates in one file isn't safe -- a later group reporting a different rate than the file's first is excluded rather than silently corrupting playback. A real, unrelated pre-existing bug in `EpubContainer::open_dir` was found and fixed while building this issue's own end-to-end test: it cloned the source directory via hard links (`clone_dir`, correct only for `Container::clone_to`, which also sets `self.cloned = true`) without ever setting that flag, so any file the container never individually rewrote stayed hard-linked to the original directory and had its *shared* inode truncated to zero bytes the moment `commit`'s own `sync_dir_tree` copied it back out -- corrupting the original source tree too. Fixed by giving `open_dir` a real independent-copy path (`copy_dir`), matching upstream's own `shutil.copy`-based behavior for this exact code path exactly. Tested end to end against a real EPUB3 fixture and real synthesized Piper speech: sentences marked, SMIL/M4A files generated, `media-overlay`/`media:duration` wired into the OPF, verified after a real commit-and-reopen round trip, then fully reversed via `remove_embedded_tts` and reverified.)
- [x] upgrade.py
- [x] utils.py
...
- [x] books.py (issue #483 closes out #427's core scope -- book-get/set-last-read-position, book-get/update-annotations (issue #485, real annotation storage + merge algorithm via `calibre_db::annotations`), the disk-cache layer (issue #482, `calibre_srv::books_cache::BookCache`), and now the real HTTP surface tying it together with the render pipeline (#481) via `calibre_srv::jobs::JobsManager` (#428): `GET /book-manifest/{book_id}/{fmt}` (cache-hit returns the rendered manifest merged with live metadata/read-positions/annotations, cache-miss queues/dedupes a render job and returns a poll-able job-status object) and `GET /book-file/{book_id}/{fmt}/{size}/{mtime}/{*name}` (serves one cached rendered asset, ETag'd with the content hash, real path-traversal guard). `mathjax`/`manifest_as_json`/`get_mathjax_manifest` closed separately as issue #484, see `calibre_srv::mathjax`. See `calibre_srv::render_endpoints`'s module doc for the disclosed `queue_job`/`job_done` simplifications)
...
- [x] legacy.py (issue #430, closed 2026-09-14 -- `calibre_srv::legacy`: `/mobile` (real search+sort+pagination HTML book listing, sort via the crate's own established ICU-collation single-field approximation of `multisort`, `crate::opds::sort_key_for`), `/browse` (old-server-URL redirect to the modern book-details panel), `/stanza` (redirect to `/opds`), `/legacy/get` (delegates to `content::handle`, strips `Content-Disposition` for old-Kindle browsers). NOT ported: custom-column "extra text" on `/mobile`'s per-book listing (needs `is_field_displayable`/`ignorable_field_keys`, neither ported anywhere in this crate) and `/static`/`favicon.png` asset serving (pre-existing gap elsewhere in this crate, not this issue's scope) -- both disclosed in the module's own doc)
...
- [x] logging.py
- [x] matcher.c (issue #462 CLOSED -- a pure C-performance reimplementation of the exact same algorithm as `matcher.py`'s `PyScorer`; not ported separately, see `matcher.py`'s own entry)
- [x] matcher.py (issue #462 CLOSED -- `calibre_utils::matcher::{Matcher,FilesystemMatcher,get_items_from_dir}`: real fuzzy subsequence-matching/scoring, incl. the real non-recursive stack-based backtracking search with `(hidx,nidx,last_idx)` memoization, word-boundary/separator/camelCase score bonuses, and the real limit-before-zero-score-filter result-truncation order. Cross-validated directly against upstream's own `test_non_bmp` test case (a non-BMP emoji character matching itself). Disclosed narrowings: `matcher.c`'s C-array memoization isn't ported separately (same algorithm, pure perf); the real `Worker`/`Queue` GIL-workaround thread pool isn't ported (Rust has no GIL, sequential scoring is the correct equivalent); `primary_find` is approximated the same documented way `calibre_db::search`'s `primary_contains` already is (NFD decompose + strip combining marks + lowercase); an empty query is treated as matching nothing rather than reproducing upstream's real `ZeroDivisionError`)
- [ ] mdns.py
- [x] network.py
- [x] ordered_dict.py
- [x] random_ua.py
...
- [x] freetype.cpp (issue #556, closed 2026-09-15 not-applicable -- backs `free_type.py`, which itself has exactly one real caller anywhere in calibre: `utils.py`'s own `test_glyph_ids` self-test. No other real consumer to port this for)
- [x] free_type.py (issue #556, closed 2026-09-15 not-applicable -- see freetype.cpp)
- [x] metadata.py (issue #556, closed 2026-09-15 -- `calibre_utils::fonts::metadata::FontMetadata`: real font-family/weight/style/stretch derivation from a font's `name`+`OS/2` tables, built entirely on `calibre_utils::fonts::{sfnt::load_font, utils::{get_font_characteristics, get_font_names2}}` (issues #548-549) with zero new low-level parsing needed)
- [x] scanner.py (issue #556, closed 2026-09-15 -- `calibre_utils::fonts::scanner`: the issue's own "GUI-and-OS-integration-adjacent, no real caller" framing was stale -- confirmed via a fresh grep that `font_scanner` is a real, non-GUI prerequisite already blocking five call sites (`oeb::transforms::embed_fonts::scan_system_font`, `oeb::transforms::flatcss::get_embed_font_info`, `oeb::polish::embed::do_embed`, `docx::fonts::has_system_fonts`, `calibre_utils::fonts::utils::get_font_for_text`), all wired for real as part of this closure. Real: directory scanning, family grouping/dedup/priority-sorting, `fonts_for_family`/`legacy_fonts_for_family`/`get_font_data`/`find_font_for_text`, plus a lazily-scanned process-wide `font_scanner()` singleton. Disclosed narrowings (module's own doc): synchronous scan instead of a background `Thread` (invisible to every real caller, which always waits for the result anyway), no persistent disk cache, `fc_list`'s fontconfig FFI query not ported (falls back to the same standard-paths list it would report anyway), Windows' `winutil`-based `font_dirs` not ported (established Windows-unverifiable pattern), and calibre's own bundled Liberation font family not vendored (a real, separate, disclosed gap, same category as issue #484's MathJax vendoring decision))
- [ ] subset.py (issue #553 CLOSED but this specific file deliberately NOT ported -- real current upstream `utils/fonts/subset.py` is a 2023 rewrite wrapping the third-party `fontTools` library's own `Subsetter`, not the hand-rolled byte-level editor this cluster's earlier filing assumed. Reimplementing fontTools itself is out of scope. `oeb::polish::subset.rs::subset_all_fonts`'s `todo!()` is instead closed via `sfnt/subset.py`'s own real, still-functional, hand-rolled algorithm -- see that file's entry below and `calibre_utils::fonts::sfnt::subset`'s module doc for the full finding)
...
- [x] winfonts.cpp (issue #556, closed 2026-09-15 not-applicable -- backs `win_fonts.py`, which has zero importers anywhere in the real upstream calibre tree, confirmed via a fresh grep. Dead code upstream itself, not just Windows-only)
- [x] win_fonts.py (issue #556, closed 2026-09-15 not-applicable -- see winfonts.cpp)
- [ ] __init__.py

##### sfnt

- [x] cmap.py (issue #551 CLOSED -- `calibre_utils::fonts::sfnt::cmap::{BmpTable,CmapTable,split_range,set_id_delta}`: real format-4 (Windows BMP) cmap subtable parsing AND writing -- `CmapTable::set_character_map` is the real encoder (contiguous-run detection, `split_range`'s consecutive-glyph-id subrange splitting with its real segment-cost thresholds, `set_id_delta`'s 16-bit wraparound math), needed for #553's subsetting to emit a NEW cmap for a glyph-renumbered font; #548's `get_glyph_ids` covers only the read-only lookup case. Reuses `fonts::utils`'s `read_bmp_prefix`/`BmpPrefix` (promoted to `pub(crate)`) and a new shared `bmp_prefix_glyph_ids` helper rather than re-deriving the same segment-walk logic a third time. Disclosed narrowings: real `CmapTable.__init__`'s vestigial always-empty `self.tables = {}` isn't ported; real `set_character_map`'s dynamic retyping of `self.bmp_table` (parsed object -> raw bytes) has no Rust equivalent -- this port re-parses the freshly built subtable back into a real `BmpTable` instead, a strictly more useful post-condition with no change to the final `raw` bytes)
- [x] common.py (issue #555 CLOSED -- `calibre_utils::fonts::sfnt::common`: the generic OpenType layout structures GSUB needs -- `Unpackable` (the relative-offset reader every layout table's arithmetic depends on), `Coverage` (formats 1 and 2) + `CoverageRange`, `LanguageSystemTable`/`ScriptTable`/`ScriptListTable`, `FeatureTable`/`FeatureListTable`, and the `read_item_sets`/`read_index_sets` pair. Upstream folds those last two into one function behind a `set_is_index` flag; split here because the two modes genuinely return different things. Confirmed again while porting: its only real importer is gsub.py, exactly as #549 found)
...
- [x] glyf.py (issue #550 CLOSED -- `calibre_utils::fonts::sfnt::glyf::{Glyph,GlyfTable}`: real simple/composite glyph parsing incl. the full composite-glyph component-record walk that recovers `glyph_indices`, and the real `glyph_data`/`update` byte-rebuilding logic 4-byte-padding each surviving glyph -- `GlyfTable::update`'s return shape is exactly what `LocaTable::update` (loca.py, same issue) expects, confirming the real calling convention between the two for #553's subsetting)
- [x] gsub.py (issue #555 CLOSED -- `calibre_utils::fonts::sfnt::gsub::{GsubTable,LookupTable,SubstitutionSubtable,Ligature}`: real GSUB glyph-substitution parsing for lookup types 1/2/3/4/5/6/7/8, and `GsubTable::all_substitutions`, the substitution closure. **REAL PAYOFF: this closes `sfnt::subset`'s own disclosed `extra_glyphs` narrowing** -- that set was plumbed through `subset_truetype`/`subset_cff` but permanently EMPTY, so any glyph reachable only through a substitution rule was silently dropped by subsetting. Now verified end-to-end: subsetting real DejaVuSans for "Hi" keeps glyph 243 (dotless i, U+0131), which no requested character maps to; a regression test confirms that test FAILS with the wiring removed. **Cross-validated against real upstream Python**: upstream's own unmodified gsub.py/common.py were executed against the real 5598-byte DejaVuSans GSUB table across 7 glyph-set cases, and this port's output matches exactly. Two faithfulness findings documented in the module: upstream's final `return ans - {glyph_ids}` is a no-op bug (subtracts a set *containing a frozenset* from a set of ints) so it really returns the full closure -- harmless, because its one caller immediately unions the inputs back in; and contextual/chaining-contextual subtables (types 5/6) contribute nothing, matching upstream's `pass  # TODO`)
- [x] head.py (issue #550 CLOSED -- `calibre_utils::fonts::sfnt::head::HeadTable` only; real upstream's `head.py` also defines `HorizontalHeader`/`VerticalHeader` (moved to #552, metrics tables conceptually), `OS2Table` (not ported separately -- already covered by #548's `get_font_characteristics`/`remove_embed_restriction`, same fields, same `fsType`-zeroing operation), and `PostTable` (deferred indefinitely, not needed by anything in this cluster's split). `FixedProperty`/`DateTimeProperty` descriptors ported as raw fields (`i32`/`i64`) plus real-value accessor methods rather than a descriptor abstraction)
- [x] kern.py (issue #552 CLOSED -- `calibre_utils::fonts::sfnt::kern::KernTable`: real classic-Microsoft-vs-Apple-format version detection heuristic, and the real `restrict_to_glyphs`/`restrict_format_0` subsetting logic that drops kerning pairs referencing a removed glyph (needed so #553's subsetting doesn't leave dangling kern references) and drops a subtable entirely once it has zero surviving pairs)
- [x] loca.py (issue #550 CLOSED -- `calibre_utils::fonts::sfnt::loca::LocaTable`: real short/long format offset reading, `glyph_location`, and the real `update` offset-table-rebuilding algorithm incl. its zero-entry-carries-forward-previous-offset convention and long/short format auto-selection. Disclosed narrowings in the module's own doc comment: the Python `array`+`byteswap` byte-order dance is unneeded (ported directly via `from_be_bytes`); `load_offsets`'s unused `maxp_table` parameter is dropped; `dump_glyphs` (print-only debug helper) isn't ported)
- [x] maxp.py (issue #550 CLOSED -- `calibre_utils::fonts::sfnt::maxp::{MaxpTable,MaxpV1Fields}`: real version-0.5/1.0 dispatch, rejects versions >1.0 matching real upstream's own check)
- [x] merge.py (issue #555 CLOSED -- `calibre_utils::fonts::sfnt::merge::{merge_truetype_fonts_for_pdf,Merged}`: real multi-font glyph/metrics merging, first-font-wins per glyph id, hhea/vhea metrics merging, hdmx/GPOS/GSUB dropped from the result. **Deliberately unwired**: upstream's only caller is `ebooks/pdf/html_writer.py`'s `merge_fonts`/`merge_font_files`, which operates on fonts extracted from an already-**Qt-rendered** PDF (`pdf_doc.list_fonts()`) -- squarely inside this port's long-standing HTML→PDF rendering-core gap (`calibre_ebooks::pdf::html_writer::convert` is a `todo!()` naming the missing browser engine). Ported anyway because it is ~90 lines, every table API it needs already existed, and it was the cluster's last unported file; tested against real DejaVu Sans/Mono, including that merging two genuinely different fonts really reports the metrics conflicts upstream logs)
- [x] metrics.py (issue #552 CLOSED -- `calibre_utils::fonts::sfnt::metrics::{FontMetrics,HorizontalHeader,VerticalHeader,Os2Table,PostTable,read_metrics,update_metrics_table}`: real `FontMetrics` assembling all 8 required tables (head/hhea/hmtx/cmap/OS-2/post/name/maxp) into PDF-embedding metrics (pdf_ascent/pdf_descent/pdf_bbox/pdf_capheight/pdf_avg_width/pdf_stemv), postscript_name resolution, underline/strikeout/overline positioning, and per-string advance-width computation via the real cmap-driven glyph lookup. Also ports `HorizontalHeader`/`VerticalHeader` (moved here from head.py, see #550's scope note) and a proper `Os2Table`/`PostTable` (the FULL real field sets -- #548's `get_font_characteristics` only covers the narrower fsType-related fields, not enough for `FontMetrics`). Cross-validated via a from-scratch-assembled minimal sfnt integration test. Disclosed narrowings: real `Sfnt.TABLE_MAP` automatic dispatch isn't wired up (each table is pulled by raw bytes via `Sfnt::get` instead, matching #550/#551's own convention); `FontMetrics.__hash__`'s process-randomized Python `hash()` is replaced with a real deterministic checksum; `postscript_name`/`advance_widths`'s real `KeyError`-raising paths are ported as `Option`/`Result` instead of a panic; `OS2Table`/`PostTable`'s dead `hasattr(...)`-based re-parse guards (which check field names that are never actually set, so never short-circuit) are omitted since real behavior always re-parses fully anyway)
- [x] subset.py (issue #553 CLOSED -- `calibre_utils::fonts::sfnt::subset::{resolve_glyphs,subset_truetype,subset,SubsetError}`: real glyph-reachability-closure TrueType subsetting -- transitive composite-glyph dependency resolution, glyf/loca/head/maxp rewriting, non-core-table stripping, cmap/kern restriction to surviving glyphs, real re-serialization. THE real payoff of this whole cluster: `oeb::polish::subset.rs::subset_all_fonts`'s `todo!()` is now closed for real, wired to this hand-rolled subsetter, incl. real `@font-face` rule cleanup for removed fonts in both standalone CSS files and inline `<style>` tags inside XHTML documents. Cross-validated end-to-end: subset a real (test-fixture) multi-glyph font down to 2 characters, confirm the surviving character still resolves via cmap, the dropped character doesn't, and the glyf table shrinks. TrueType (glyf-flavored) only -- CFF-flavored OpenType subsetting is issue #554. Real scope correction found while researching this issue: current real upstream's `utils/fonts/subset.py` (the function `oeb/polish/subset.py` actually calls) is a 2023 rewrite delegating to the third-party `fontTools` library, NOT the hand-rolled `sfnt/subset.py` algorithm this issue's own filing assumed -- ported the older, still-real, still-tested hand-rolled algorithm instead of reimplementing fontTools itself; see the module's own doc comment for the full finding and its disclosed narrowings (CFF branch was out of scope here and closed by #565; GSUB substitution closure was out of scope here and **closed by #555** -- see gsub.py's entry; CLI-only chars/ranges string parsing not ported))
...
- [x] table.py (issue #564 CLOSED -- `calibre_utils::fonts::sfnt::cff::table::{Cff,Charset}`. Real `Index` (the generic count-prefixed variable-offset-width CFF INDEX structure, incl. the real 3-byte/24-bit offset form via a zero-padded big-endian read), `Strings` (an `Index` prepended with the 391 Standard Strings), `Charset` (3 real predefined charsets + font-embedded formats 0/1/2, incl. the real format-1/2 range-expansion algorithm), and the top-level `CFF` reader tying everything together via #563's `Dict::decompile`. `Charset::lookup`/`safe_lookup` collapse to one method here since this port's `lookup` already returns `Option` and never panics, unlike real upstream's raise-then-catch pair. Scope: the reader only -- `CFFTable`'s own container-wiring and `subset()` (which needs the not-yet-built `writer.py`) are #565's job. Real upstream's CID-font early-bailout (`is_CID` raises `UnsupportedFont` before `Charset` is ever constructed for one) is preserved, making `Charset`'s own CID-name-formatting branches technically unreachable via the one real caller but kept for fidelity to the class's real public contract.)
- [x] writer.py (issue #565 CLOSED, depends on #564 and the already-closed #553 -- `calibre_utils::fonts::sfnt::cff::writer::{IndexBuilder,StringsWriter,CharsetsWriter,Subset}`. `Subset` (the real subsetting orchestrator) resolves the interior-mutability gap left open by the prior partial pass via a `RefCell<StringsWriter>` captured by the `resolve_sid` closure passed to `dict_data::Dict::compile`; ports the real two-pass Top-DICT offset-patching dance (compile with dummy offsets to learn size, recompile with real ones -- offsets always use the fixed 5-byte form so size doesn't change between passes) and the glyph-keep-set-driven CharStrings rebuild (dropped glyphs become a single real `endchar` operator byte, preserving glyph ids). A real gap found while porting: real `writer.py`'s own `Dict` class subclasses `Index` and wraps the compiled Top DICT byte-code as the sole entry of a one-item INDEX before use -- #563's `dict_data::Dict::compile` (by design) only produces the bare byte-code, so `Subset::new` does that wrapping itself (`compile_top_dict_index`), caught by a reparse-through-the-real-reader test failing with "Unsupported CFF INDEX offset size: 0" before it was added. A real upstream quirk faithfully replicated rather than fixed: `PrivateDict.compile`'s local-Subrs offset is computed from a compile pass that ran *before* the `Subrs` field itself was added to the dict, so the stored offset doesn't account for the size the field's own entry adds -- only matters for fonts with local (non-global) Subrs, uncommon for embedded ebook fonts. `CFFTable.subset` ported as `calibre_utils::fonts::sfnt::subset::subset_cff`, wired into `sfnt::subset::subset`'s CFF branch (previously `UnsupportedFont`) -- maps cmap-derived glyph ids to glyph names via the font's own `Charset`, subsets, remaps `character_map` back (faithfully replicating real upstream's `if glyph_id:` Python-truthiness check that never maps a code to glyph 0). Real, tested via reparse-through-the-real-#564-reader round trips at both the `Subset` and `subset_cff` levels, not a second hand-check.)
...
- [x] imageops.cpp (issue #67 split into #569/#570/#571 -- ALL 4 sub-functions of #570 now CLOSED too, alongside #569's earlier work. `calibre_utils::imageops::{remove_borders,grayscale,overlay,has_transparent_pixels,set_opacity,texture_image,dominant_color,normalize}` (#569) plus `{gaussian_sharpen,gaussian_blur,despeckle,oil_paint}` (#570), all ported against `image::RgbaImage`. #569 found and fixed a real bug in `overlay`/`texture_image`'s alpha-blend math while writing tests: the premultiplied source's ALPHA channel was being re-multiplied by itself instead of used as-is -- caught by a real test expecting the opaque canvas to stay opaque after blending, not by review. #570's own real, confirmed-not-a-bug quirks, each verified by independently re-deriving upstream's own pointer-arithmetic C algorithm from scratch and reproducing bit-identical numeric results before trusting the Rust port: (1) `gaussian_blur`'s real "kernel wider than the image" branch indexes the kernel by SOURCE column for the weighted sum but by a DIFFERENT, shifted index for its own renormalization scale -- a real upstream inconsistency that makes a uniform-color image NOT a fixed point in that branch (only in the normal kernel-fits-in-image case); (2) `despeckle`'s 1-pixel zero-padded working buffer means a uniform image erodes toward 0 right at the border (real upstream zero-initializes that buffer too) -- the interior, far from the border, is a genuine fixed point; (3) `despeckle` only mildly attenuates an isolated single-pixel outlier in one pass (e.g. 255-on-0 settles around 239), not a flood-fill removal -- its real per-hull-call magnitude is at most ±1 per direction, gated by strict neighbor comparisons. `quantize`/eink-dither (`quantize.cpp`/`ordered_dither.cpp`) are #571, now CLOSED too -- the entire `imageops` cluster (#67) is complete. See those files' own entries below)
- [x] imageops.h (see imageops.cpp above -- the shared declarations, split the same way)
- [x] imageops.sip (SIP/PyQt binding glue, N/A -- this port has no Python/Qt binding layer)
- [x] ordered_dither.cpp (issue #571 CLOSED -- `calibre_utils::quantize::ordered_dither`, a real ImageMagick-derived 8x8 ordered dither quantizing to the 16-level e-ink palette, exact integer-only math preserved)
- [x] quantize.cpp (issue #571 CLOSED -- `calibre_utils::quantize::quantize`, a real octree color quantizer with Floyd-Steinberg serpentine dithering, represented as a `Vec<Node>` arena rather than upstream's raw-pointer memory pool (matching this crate's own established `Dom`-arena pattern). Mirrors real `calibre.utils.img.quantize_image`'s own wrapper behavior (blend transparency onto white first via already-real `overlay`) rather than the raw C++ `imageops.quantize`'s stricter "throw if any alpha channel" precondition, since `image::RgbaImage` always structurally carries alpha the way Qt's `Format_ARGB32` does -- a literal port would reject every real image unconditionally, which isn't upstream's actual observable behavior for its own real callers (who always go through the pre-blending Python wrapper). **Real bug found and fixed, not reproduced**: upstream's own `create_child`/`add_color` create tree leaves one level later than `MAX_DEPTH` (8) supports -- at the common `max_colors >= 256` case, this makes the octree call `get_index(..., level=8)`, one past the real 8-entry `BIT_MASK` bound, an out-of-bounds C++ array read (undefined behavior in the original, not reproducible or worth reproducing in safe Rust). Fixed by capping the effective tree depth at `MAX_DEPTH - 1`; the quality cost (7 vs. 8 bits of octree branching) is negligible for real images. Disclosed, not ported: the pre-quantized (`Format_Indexed8`) input path -- no real caller in this crate ever constructs an already-indexed image, only full RGB(A) ones)

#### ipc

- [x] job.py (issue #573, closed 2026-09-15 not-applicable -- `BaseJob`/`ParallelJob`'s only real non-GUI caller anywhere in calibre is `ebooks/comic/input.py`'s `ComicInput`, whose Rust port (`calibre_ebooks::comic::page_processor`, issue #124) already replaced the `Server`/`ParallelJob` subprocess pool with a `std::thread::scope`-based one -- a deliberate, already-shipped different design, not a gap. Every other caller is `gui2/`'s job-list widget, which this port has no equivalent of)
...
- [x] server.py (issue #573, closed 2026-09-15 not-applicable -- see job.py)
...
- [x] piper.py (issue #77 CLOSED -- split into #610/#611/#612, all CLOSED, after a feasibility spike found the "2 files" framing understated the real scope: a full neural TTS pipeline (eSpeak phonemization + Piper ONNX vocoder + Qt audio playback), none of which previously existed in this workspace. #610 shipped as `crates/calibre_ebooks/src/tts/piper.rs`: `VoiceConfig`/`translate_voice_config`/`load_voice_config`/`create_voice_config` (real Piper voice-config JSON parsing -- more robust than real Python, which would raise `AttributeError` on a missing `"inference"` object) plus real phonemization + phoneme-to-ID-sequence building (`text_to_sentence_ids`) replicating `piper.cpp`'s exact `BOS,PAD,<id,PAD>*,EOS` algorithm, using `espeak-ng` (a genuine pure-Rust port of eSpeak NG, not an FFI wrapper -- found after a real FFI alternative, `espeak-rs`, hit build bugs in its own vendored eSpeak-ng snapshot). Disclosed narrowing: this port splits clauses itself (a direct, real punctuation-based split) and phonemizes each separately, re-inserting the same literal terminator characters `piper.cpp` does, rather than relying on eSpeak-ng's own low-level per-clause API (unavailable in the pure-Rust crate) or its simplified `read_clauses` approximation (confirmed via the crate's own doc comments not to match the real upstream `ReadClause()` algorithm). #611 shipped as `crates/calibre_ebooks/src/tts/vocoder.rs`: real ONNX inference via `ort` (real ONNX Runtime bindings), NOT `tract-onnx` as #610's own plan assumed -- verified with a real downloaded Piper voice (`en_US-lessac-low`, fetched from HuggingFace for this spike, not committed to the repo) that `tract-onnx` fails real symbolic shape inference on Piper's actual VITS-style attention graph (a genuine `tract` limitation, not a wiring bug), while `ort` (the same choice the existing `piper-rs` crate makes) loads and runs the same model correctly -- confirmed by generating and listening to real synthesized speech end to end before committing to the approach. A real upstream bug was found and NOT reproduced: `piper.cpp`'s `next()` allocates its float-output buffer as `num_samples * num_of_silence_samples` instead of `+`, a real buffer-overflow-prone bug whenever `sentence_delay > 0`; this port always uses the correct `+` sizing. #612 shipped as `crates/calibre_ebooks/src/tts/stream.rs`: the real `Piper` class -- a background-thread work queue with real generation-guard semantics (every job tagged with the `voice_id` current when queued; `set_voice`/`cancel` bump the counter, silently dropping stale queued/in-flight work), streaming `SynthesisResult` chunks through an `mpsc` channel. A real self-introduced deadlock was found and fixed before shipping: `shutdown()` must drop its own `Sender` *before* joining the worker thread, not after -- joining first blocks forever since the worker's `recv()` only returns once every sender is gone. Scoped out: `global_piper_instance()`'s `atexit`-based singleton (Rust `static`s aren't dropped at normal process exit the way Python's `atexit` guarantees, so a naive port would silently skip the real graceful-shutdown-on-exit behavior). All TTS integration tests need a real `.onnx` voice file via `CALIBRE_OXIDE_TEST_PIPER_VOICE` (skip gracefully otherwise, since 20-60MB model files aren't committed to source control) -- verified passing against a real downloaded voice before shipping each of #611/#612)
- [x] __init__.py (N/A -- empty package marker)
...
- [x] news.py (issue #81 CLOSED, split into #617 (this file's `Feed`/`Article` prerequisite, CLOSED) / #618 (templates.py, CLOSED) / #619 (config+hooks+readability, CLOSED) / #620 (HTML cleanup, CLOSED) / #621 (cover/masthead glue, CLOSED) / #622 (create_opf, CLOSED) / #623 (fetch+concurrency orchestration, CLOSED -- whole web/feeds cluster now complete). See `__init__.py`'s own entry for #617's real findings. #619 shipped as `crates/calibre_ebooks/src/web/feeds/recipe.rs`: the real ~55-field `RecipeConfig` config surface, `NewsRecipeHooks` trait (the ~20 trivial hook methods, as trait methods with real default bodies), and `extract_readable_article` wired directly onto the already-real `readability::Document`. Tag-selector fields (`remove_tags`/`keep_only_tags`/etc.) use a minimal, real `TagSpec` data shape -- matching logic itself is #620's job. Disclosed narrowing: the "does the extracted title already appear in the body" check (deciding whether to insert a synthetic `<h2>`) uses subtree text-content search rather than real Python's per-element exact-direct-text-equality check -- only affects whether a cosmetic redundant heading gets inserted. #620 shipped as `crates/calibre_ebooks/src/web/feeds/postprocess.rs`: `_postprocess_html`'s real fixed cleanup pipeline (stylesheet/script/bad-tag removal, override-CSS injection, real navbar insertion by grafting a separately-rendered `templates.rs` page's `<div>` into the article's own `Dom` via a new `import_subtree` cross-arena copy helper, HTML5-semantic-tag nuking to `div.calibre-nuked-tag-X`). Real finding, confirmed by reading `news.py`'s own `download`/`__init__` methods, not assumed: `remove_tags`/`keep_only_tags`/`remove_tags_after`/`remove_tags_before`/`preprocess_regexps` are NOT applied inside `_postprocess_html` at all -- they're merely copied onto `web2disk_options` for `RecursiveFetcher` (issue #455) to apply during the actual fetch, so #620's real scope is meaningfully narrower than #81's original scoping pass assumed. `populate_article_metadata` is out of scope here too, deferred to #623 (needs live `Feed`/`Article` state this pure `Dom -> Dom` transform has no access to). #621 shipped as `crates/calibre_ebooks/src/web/feeds/cover.rs`: real `download_cover`/`default_cover`/`download_masthead`/`resolve_masthead`/`default_masthead_image` wiring the already-real `crate::scraper::Browser` (fetch), `crate::covers::{create_cover, generate_masthead}`, and `crate::web::fetch::utils::prepare_masthead_image` together, plus a new, real, narrow slice of `calibre.utils.img` this cluster needed and nothing else in the crate provided yet (`add_borders_to_image`/`image_to_data`/`save_cover_data_to`, backed by the `image` crate). Added `get_masthead_title` to `NewsRecipeHooks` (trivial hook, defaults to the config title). Disclosed narrowing: `_download_cover`'s PDF-cover-metadata-extraction branch (`ext == 'pdf'`) has no real equivalent -- no PDF metadata/cover-extraction path exists anywhere in this crate's `pdf` module yet -- so a PDF-extension cover URL now resolves to "no cover," same observable outcome as real Python's own `get_metadata` returning no `cover_data`. Disclosed narrowing, inherited from #619's own hook shape: `get_cover_url`/`get_masthead_url` returning an already-open file-like object has no reachable equivalent, since those hooks return `Option<String>`; the local-path and remote-URL branches are both real. #622 shipped as `crates/calibre_ebooks/src/web/feeds/opf.rs`: real `build_periodical_toc` (pure, directly testable against synthetic downloaded-article fixtures -- feed/article play-order tree, internal-toc-entry sub-items, sub-page spine ordering, single-feed-vs-multi-feed TOC shape) and `create_opf` (assembles `MetaInformation`+manifest+guide+spine, reusing already-real `opf_writer::{write_opf,write_ncx,auto_manifest,scan_directory_manifest}`, wired onto #621's `cover::default_cover` for the no-cover-yet fallback). Extended shared `metadata::toc::{TOC,TOCNode}` with real `play_order`/`author`/`description`/`toc_thumbnail` fields and a real `add_item` (matching real Python's own `calibre.ebooks.metadata.toc.TOC` shape exactly, previously only a read-side NCX parser); `opf_writer::write_ncx` now renders these as `<calibre:meta>` NCX extension elements and honors an explicit `play_order` (auto-numbering only when unset, preserving every pre-existing caller's behavior); `opf_writer::write_opf` gained an `ncx_href` parameter (periodicals need `index.ncx`, not every other format's `toc.ncx`); `MetaInformation` gained `publication_type` (rendered as `calibre:publication_type`) and now renders its existing but previously-unused `author_sort` field. Also ported `BasicNewsRecipe.description_limiter` for real (`recipe.rs`; previously an injected closure placeholder in `templates.rs`, issue #618). Real, previously-inert `Article::internal_toc_entries` (dead since #617, no consumer until now) changed from `Vec<String>` to a real `anchor`/`title` shape; added `Article::orig_url`/`sub_pages` (set by real fetch orchestration, #623). Disclosed narrowing, deferred to #623: real `create_opf`'s bottom-navbar re-injection into already-saved article HTML files (distinct from `_postprocess_html`'s top navbar, #620) and `article_url_map`/`canonicalize_internal_url` tracking (consumed only by `internal_postprocess_book`, a third, separate OEB-conversion-time hook not mentioned in #622's own scope and not ported) both need live orchestration state this pure TOC/OPF builder doesn't have -- `generate_navbar(bottom: true, ...)` is already real and ready in `templates.rs` for #623 to use. Disclosed narrowing: `article_titles`/`mi.comments` includes every article's title regardless of download outcome, since this port has no `aborted_articles`/`failed_downloads` tracking yet (cosmetic only). Disclosed narrowing: `periodical_date_in_title` (a real `OutputProfile` field this port has no profile-selection system for) is the caller's own responsibility, matching #618's own established `date_str`-not-`timefmt` convention). #623 shipped as `crates/calibre_ebooks/src/web/feeds/download.rs`: real `parse_feeds` (fetches every feed URL via `scraper::Browser`, parses via #617's `feed_from_xml`, per-feed errors become an error-placeholder `Feed` rather than aborting the whole run), `remove_duplicate_articles` (dedup by title/url per #619's two real booleans), `feeds2index`/`feed2index` (wiring #618's templates), and `build_index` -- the real, end-to-end orchestrator: parses feeds, downloads cover/masthead (#621), writes index.html/feed_N/index.html, downloads every article concurrently via a bounded worker-pool (`std::thread::scope` + shared queue, matching this crate's own established `oeb::polish::download` pattern) built on #455's whole `RecursiveFetcher` stack (#630-#633), then assembles the final OPF/NCX (#622). `FetcherContext`s share one `image_cache`/`stylesheet_cache` pair across every worker thread, exactly as real Python shares `imagemap`/`stylemap` across sibling `RecursiveFetcher` instances -- reusing the cross-instance sharing #633 already verified works. Verified end-to-end against a real small periodical (a real RSS feed + two real linked articles, served by a hand-rolled test HTTP server), not just unit-level pieces in isolation. Disclosed narrowings: `parse_index()`-based custom index scraping and `get_obfuscated_article` have no hooks in this port (neither is modeled by `NewsRecipeHooks`), so custom-index recipes always fall through to `parse_feeds` and `articles_are_obfuscated` is effectively unreachable; HTTP Basic-auth embedded in a feed URL has no equivalent (`Browser` doesn't model per-request credentials), such a feed just fetches unauthenticated and likely 401s, handled like any other feed-fetch failure; one shared `Browser` is used across every worker thread rather than real Python's one-clone-per-article-thread (no `Browser` clone/fork operation exists yet), so concurrent article fetches share cookies -- only matters for stateful/login-requiring recipes, not the common read-only case)
- [x] templates.py (issue #618 CLOSED, split from #81 -- `crates/calibre_ebooks/src/web/feeds/templates.rs`: real HTML generation for all 8 template classes via a small internal element-tree builder (lxml's text/tail idiom replicated by pushing a `Node::Text` sibling immediately after an element -- no special-cased "tail" concept needed). Disclosed narrowing: always serializes as HTML rather than switching HTML/XML serialization modes per `IS_HTML`; `pretty_print` whitespace formatting not replicated (cosmetic only). Found and did NOT reproduce real, confirmed dead code: `FeedTemplate`/`TouchscreenFeedTemplate`'s `getattr(feed, 'image', None)` branch can never execute in real upstream, since `Feed.__init__` (issue #617) never sets a `self.image` attribute anywhere -- confirmed by reading `__init__.py` directly, not assumed)
...
- [x] simple.py (`RecursiveFetcher` -- issue #455 (epic), split into #630 (`fetch_url`+URL/link-filtering primitives, builds on already-real `scraper::Browser` -- CLOSED, `crates/calibre_ebooks/src/web/fetch/simple.rs`) / #631 (HTML preprocessing pipeline + tag-selector matching engine -- CLOSED, `crates/calibre_ebooks/src/web/fetch/get_soup.rs`) / #632 (image/stylesheet download+rewrite -- CLOSED, `crates/calibre_ebooks/src/web/fetch/media.rs`) / #633 (the recursive crawl engine itself, depth limiting, cross-instance shared caches -- CLOSED, `crates/calibre_ebooks/src/web/fetch/recursive.rs`; #623 now unblocked). Scoped out, no real consumer found (grepped every `RecursiveFetcher`/`create_fetcher`/`web2disk` call site in `old_src`; only real caller is `news.py`/#623): `option_parser`/`create_fetcher`/`main` (the `web2disk` CLI entry point). `crate::oeb::polish::download` (a different, unrelated port) is NOT reusable here despite the superficial similarity -- different input shape (flat known URL list vs. discovered link graph), no retry, wrong concurrency granularity, and it deliberately rejects HTML documents (the opposite of what a page crawler needs). #630's real API adaptation: `Browser::open`/`open_novisit` return `Ok` regardless of HTTP status, so `fetch_url` itself inspects `resp.status()`/`resp.reason()` to produce the same `FetchError` real Python gets from a raised `URLError`/`HTTPError`. #631 grepped all 1077 real `.recipe` files' actual `remove_tags`/`keep_only_tags` shapes before designing the matcher (per its own scoping note) and found real gaps in #619's original `TagSpec`: widened `name` from a single `Option<String>` to `Option<Vec<String>>` (71+ real recipes pass multiple names), added `text`, folded every implicit top-level `dict(id=..., class_=..., ...)` keyword into `attrs` (not just an explicit `attrs={}`), and widened `RecipeConfig::remove_tags_after`/`remove_tags_before` from `Option<TagSpec>` to `Vec<TagSpec>` (real Python accepts either a single dict or a list; ~50 real recipes use a list). Deliberately NOT supported, disclosed: regex-valued or lambda-valued `attrs` (real but rare, a few dozen of ~1400 real uses) -- not representable in a static Rust data shape without a small expression language of its own. #632's real fetch/image_url_processor/preprocess_image dependencies are injected as closures rather than hard dependencies on `Browser`/`NewsRecipeHooks`, keeping the module testable against a stub fetcher; disclosed simplification: `process_images`'s `data:` URL branch uses the same `fetch` closure as every other branch rather than real Python's separate raw `urlopen(iurl).read()` call, since both decode to the same bytes. #633 ties #630/#631/#632 together via explicit `FetcherContext`/`FetchState` parameters rather than `&mut self` methods (sidesteps real borrow-checker friction a direct method-based port would hit); shares `image_cache`/`stylesheet_cache` across independent `FetcherContext`s exactly as real Python shares `imagemap`/`stylemap` across sibling per-article `RecursiveFetcher` instances (verified with a real dedup-across-two-contexts test, not just a single-instance one); disclosed narrowing inherited from #620's own hook shapes, not newly introduced: `AbortArticle` has no reachable raise path here since `preprocess_html`/`postprocess_html` return a plain `Dom`, not a `Result`, so every per-link failure uniformly becomes a `failed_links` entry, matching Python's own *other* exception branch)
...
- [x] parser.py (issue #452 CLOSED, depends on #451 -- `calibre_ebooks::css_selectors::parser`. Real hand-rolled tokenizer (NOT `cssparser` -- a genuine, load-bearing divergence found while porting: `cssparser`'s CSS Syntax Level 3 tokenizer merges a digit run immediately followed by ident chars into one `Dimension` token, e.g. `2n` -> one token, but real upstream's own regex tokenizer always splits it into `NUMBER "2"` + `IDENT "n"`, exactly the shape `:nth-child(2n+1)`'s `parse_series` needs; silently reusing `cssparser` would have broken every structural pseudo-class argument). The parsed tree (`Element`/`Class`/`Hash`/`Attrib`/`Pseudo`/`Function`/`Negation`/`CombinedSelector`) is one recursive `Node` enum instead of a class hierarchy. Cross-validated directly against the live real Python function via a temporary stub `css_selectors` package (pointing only at `errors.py`+`parser.py`, sidestepping `select.py`'s `lxml` import) over ~35 real selector strings -- caught one real bug this way: `Selector.__init__`'s own pseudo-element lower-casing applies regardless of which of the two real parsing branches produced it, so `:Before` (CSS2.1 single-colon form) really does lower-case to `"before"` even though that branch's own local variable is untouched at assignment time -- an initial reading of the branch in isolation looked like it preserved case and was wrong; the xval battery caught it immediately.)
- [x] select.py (issue #453 CLOSED, depends on #452 -- closes the entire #85 tracking issue. `calibre_ebooks::css_selectors::select`: a new, tree-agnostic `Element` trait (separate from `crate::css::matcher::Element`, the narrower #164 engine's own trait) implemented for `DomElement` (over `crate::dom::Dom`), plus `Select<E>` -- the real matching engine, combinators, attribute operators, structural pseudo-classes, `:lang()` (incl. `normalize_language_tag`'s real BCP47 subtag-combination algorithm), `:not()`, `:root`. Dispatch is by `match` on the real AST/name instead of upstream's string-keyed `dispatch_map`; caches are built eagerly in `Select::new` instead of upstream's lazy `@property` memoization; document order is achieved by a single final filter pass over the real document-order element list rather than threading order through every intermediate generator. Two real findings: (1) `attribute_operator_mapping` has no `!=` entry even though the grammar parses it, so real Python raises an uncontrolled `KeyError` for `[attr!=x]` -- confirmed by triggering it directly against the live function -- ported as a real, controlled `SelectorError::Expression` instead; (2) `lang_map`'s own loop unconditionally overwrites `lang` when checking `xml:lang` then `lang`, so the plain `lang` attribute always wins regardless of which is actually set -- looks like a real upstream bug (probably meant to prefer `xml:lang`), replicated bug-for-bug per this project's convention rather than "fixed". Cross-validated against the live real Python `Select` class via a fake `lxml.etree`/`El` tree (the established lxml-shaped xval technique) over ~30 real selector/tree combinations spanning every combinator, attribute operator, structural pseudo-class, `:not()`, `:root`, `:lang()`, and the negative-`a`-coefficient `:nth-child(-n+3)` case -- every case matched with no further bugs found.)
- [x] tests.py (upstream's own test suite, not ported line-by-line -- #452/#453 write real Rust tests covering the same grammar/matching surface instead, plus direct cross-validation against the live real Python for both)
