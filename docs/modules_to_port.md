...
- [ ] to_html.py (`docx/to_html.rs`'s `DOCXToHTML` is still the provisional sketch, wired into `input/docx_input.rs`; every per-piece function is ported, plus a `convert_document` orchestrator sequencing `read_page_properties`/`convert_body`/`read_block_anchors`/`mark_block_runs`/`convert_footnotes`/`apply_tab_indentation`/`resolve_links`/`cascade`/`apply_tables_markup`/`apply_numbering_markup`/`apply_block_run_frames`/`apply_paragraph_frames`/`assign_style_classes`/`cleanup_markup`/`create_toc` in `Convert.__call__`'s real order, plus `Convert.read_styles`'s package-loading step (`docx/read_styles.rs`'s `read_raw_parts`/`wire_parts`, split into a real-I/O half and a wiring half to sidestep a multi-document lifetime problem) and `Convert.write`'s OPF/NCX/index.html output (`build_html_document` ports `Convert.__init__`'s `<html><head>` skeleton construction; `write_document` writes `index.html`/`metadata.opf`/`toc.ncx`, built on a crate-wide `opf_writer.rs` — relocated out of `mobi/` and generalized with a new `scan_directory_manifest` helper both `mobi8.rs` and this now share — plus a new `Toc::to_ncx_toc` adapter from `docx/toc.rs`'s own arena into `metadata::toc::TOC` — `docx.css` generation is skipped, same `fonts.py`/`generate_css` gap as `styles.py`) — and images wired into `convert_run`/`convert_p`/`convert_body`/`convert_footnotes`/`convert_document` via `Images::to_html` (real `<img>`/`<hr>` markup, real `PageProperties` threaded through for float positioning, real relationships per footnote/endnote) — plus `convert_docx_document`, a real top-level entry point (`Convert.__call__`'s full equivalent, including its own `self.write(doc)` call at the end) that opens a package, resolves/parses every part via `read_raw_parts`/`wire_parts`, runs `convert_document`, and returns `write_document`'s `metadata.opf` path — not yet wired into `DOCXToHTML`/`input/docx_input.rs` (that plugin expects an HTML *string* it hands to `HTMLInput`, not an OPF-rooted book — needs either a general OPF-based book loader or a narrower adapter, a separate follow-up); remaining scope beyond that wiring gap: `resolve_alternate_content` (source-tree mutation, same open question as #290/#293) and `fields.py`'s orchestrator (#290) — see #130/#288)
