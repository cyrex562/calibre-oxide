...
- [x] plumber.py (`Plumber::run()`'s transform stage was a no-op placeholder despite `oeb::transforms::*` already having 20 real, tested modules from issue #41 — issue #686 filed to track wiring the real pipeline in; first slice shipped here: `DataURL`/`Clean`(guide)/`DetectStructure`/`ManifestTrimmer`, the option-independent stages. Remaining stages (`MergeMetadata`/`Jacket`, `CSSFlattener`, font embedding/subsetting, output-plugin `opts`/`log` threading) each need their own real prerequisite — see #686's own breakdown. Blocks #126 (CLI option assembly), which needs a real destination for parsed options before it's worth building.)
- [x] preprocess.py
- [x] search_replace.py
- [x] utils.py
