...
- [ ] news.py (partial -- issue #81 CLOSED, split into #617 (this file's `Feed`/`Article` prerequisite, CLOSED) / #618 (templates.py, CLOSED) / #619 (config+hooks+readability, CLOSED) / #620 (HTML cleanup) / #621 (cover/masthead glue) / #622 (create_opf) / #623 (fetch+concurrency orchestration, blocked by #455). See `__init__.py`'s own entry for #617's real findings. #619 shipped as `crates/calibre_ebooks/src/web/feeds/recipe.rs`: the real ~55-field `RecipeConfig` config surface, `NewsRecipeHooks` trait (the ~20 trivial hook methods, as trait methods with real default bodies), and `extract_readable_article` wired directly onto the already-real `readability::Document`. Tag-selector fields (`remove_tags`/`keep_only_tags`/etc.) use a minimal, real `TagSpec` data shape -- matching logic itself is #620's job. Disclosed narrowing: the "does the extracted title already appear in the body" check (deciding whether to insert a synthetic `<h2>`) uses subtree text-content search rather than real Python's per-element exact-direct-text-equality check -- only affects whether a cosmetic redundant heading gets inserted)
