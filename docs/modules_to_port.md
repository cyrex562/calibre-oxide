...
- [ ] from_html.py (see #132 — its "needs a real OEB stylizer" premise was stale: `oeb/polish/cascade.rs`'s real CSS cascade already existed, just for a different consumer (issue #164); `oeb/polish/style.rs` now ports the accessor half of `stylizer.py`'s `Style` class against it — `get`/`own`/`item`/`color`/`background_color`/`font_size`/`line_height`/`effective_text_decoration`/`first_vertical_align`/`is_hidden`/`width`/`height`/margins/paddings — the exact surface `styles.py`/`from_html.py` read. `TextRun`, `Block`, `Blocks` (partial, see below), `lang_for_tag`, and the core `Convert` element walker (`create_block_from_parent`/`add_block_tag`/`add_inline_tag`/`process_tag`/`process_item`) all ported to `docx/writer/from_html.rs`, resolving `cascade::resolve_styles` (already real, `oeb/polish/stats.rs` already uses it) per element via `oeb::polish::pretty`'s existing `leading_text`/`dom_tail` helpers for lxml's text/tail model. `tagname == 'img'` (needs `images.py`, unported) and table-typed `display` (needs `tables.py`, unported, and `Blocks`' still-unported table methods) are documented `todo!()`s; `<hr>`'s border-suppression override is a disclosed, narrower gap (no `Style` mutation capability exists). `Blocks`' `Table`-related methods and `.serialize`, `Convert.__call__`/`.write` (need `ImagesManager`/`ListsManager`/`StylesManager.finalize`, none ported, plus a real float-group-membership design decision), and the `Style`/`Stylizer` subclasses (subsumed by `oeb/polish/style.rs`/`cascade.rs`) are not ported yet)
- [ ] images.py (see #132; also needs `pt_to_emu` from the reader's `images.py`, already ported — #130. Partial: the self-contained utility layer — `get_image_margins`, `create_filename`, `create_docx_image_markup` — is ported to `docx/writer/images.rs`, plus `Style.img_size`/`img_dimension` ported to `oeb/polish/style.rs`. `ImagesManager`'s data-source half is NOW ported too: `Image` (narrowed to an `href: String` instead of holding a whole `ManifestItem` — matches this effort's established "narrow to what's read" pattern) and `read_image`/`read_svg` (manifest lookup with a urlquote-fallback, real byte content via `OEBBook.container.read(href)`, format/dimension detection via the already-ported `calibre_utils::imghdr::identify`, a corrupted-image fallback to calibre's bundled `images/blank.png` via `calibre_utils::resources::get_image_path`, and relationship registration via the already-ported `DocumentRelationships::add_image`). **This resolves the real design question the previous entry flagged**: where real image byte content comes from — confirmed by grepping every other writer in this crate (`mobi::writer2`/`writer8`, `oeb::transforms::{guide,data_url,htmltoc,metadata}`, `input::{rb,snb}_input`) that they all read manifest item bytes via `OEBBook.container: Box<dyn oeb::container::Container>`'s `.read(href)` — the SAME abstraction `oeb::reader.rs` uses to load the OPF itself. This is a DIFFERENT `Container` than `oeb::polish::container::Container` (the filesystem-backed "Polish Book" editor, issue #163) despite the identical name — confirmed by reading both definitions, not assumed. No new abstraction was needed at all. Still not ported: `create_image_markup`/`add_image`/`serialize`/`create_cover_markup`/`write_cover_block` (need a `Style`/stylizer context, real DOM `<img>`/`Block` integration, write-time package access, and the cover-image resolution flow from `Convert.__call__` respectively — none blocked on a design question anymore, all real implementation work) and `SvgRasterizer`'s `svg_originals` tracking (not part of the existing `oeb::transforms::rasterize::SvgRasterizer` port). The `Element::insert`-at-front capability `write_cover_block` needs already exists in `docx/writer/xml.rs`, ported for `links.py`'s TOC half)
...
- [ ] tables.py (see #132; partial: `Border`, `border_style_weight`, `as_percent`, `convert_width`, and a `Border`-repackaging `read_css_block_borders` (thin wrapper over `styles.rs`'s already-ported one) all ported to `docx/writer/tables.rs`. `SpannedCell`/`Cell`/`Row`/`Table` are ported, held in a `Tables` arena (`TableId`/`RowId`/`CellId` handles into `Vec<Table>`/`Vec<Row>`/`Vec<CellSlot>`, `CellSlot` a real `Cell(Cell)`/`Spanned(SpannedCell)` enum standing in for Python's duck-typed `isinstance` checks) — one arena per level, cross-referenced by id, not one shared tagged arena. `Tables::neighbor`/`::applicable_borders`/`::resolve_border`/`::resolve_cell_borders` (CSS table border-conflict resolution) and `::expand_spanned_cells` (merge-continuation placeholder insertion for `rowspan`/`colspan`'d cells) are ported and tested in isolation. On top of these, `Tables` NOW also ports the full stateful mutation half every real HTML-walking caller needs — `table_start_new_row`/`table_start_new_cell`/`row_start_new_cell` (port of `Table`/`Row.start_new_row`/`.start_new_cell`), `table_finish_tag` (port of `Table.finish_tag` including the row-level `Row.finish_tag` layer — returns whether the table's own tag closed, triggering `expand_spanned_cells`+`resolve_cell_borders` for every real cell), and `table_add_block`/`table_add_table`/`row_add_block`/`row_add_table`/`cell_add_block`/`cell_add_table` (port of `Table`/`Row`/`Cell.add_block`/`.add_table`, including their auto-create-a-cell/row fallbacks, which reconstruct a `Style` on demand from a stored `NodeId` — `Row`/`Table::orig_tag_style_node` — the same pattern used throughout this effort rather than trying to store a live `Style` object the way Python does). `Cell` gained a real `items: Vec<ItemId>` field, `ItemId` a new `Block(BlockId)`/`Table(TableId)` enum (defined in `from_html.rs`, shared with `Blocks`' own still-unported top-level `items` list) standing in for Python's `Cell.items`/`Blocks.items` mixing `Block`/`Table` objects directly. Still NOT ported: `Blocks`' OWN half of the integration (its `tables` stack of currently-open tables, `finish_tag`'s table-closing branch, and widening `Blocks.items` from `Vec<BlockId>` to `Vec<ItemId>` — needs `Block` to gain a real "which container" field too, replacing the dropped `parent_items`, likely `enum ItemContainer { Top, Cell(CellId) }`, so `Blocks::delete_block_at`/`apply_page_break_after` can tell which items-list a block lives in). None of `Cell`/`Row`/`Table`/`Blocks.serialize` are ported either, since they need real content from that same deferred integration)
